...
| `events.jsonl` | `logForensicEvent()` (Phase I+) | Append-only JSON Lines forensic log. One line per incident: rate_limit, auth_failure, server_error, client_disconnect, queue_saturation, inflight_escalation, dashboard_start, etc. Each entry carries a stable schema (ts + category + typed payload) so jq / awk / grep work without parsing. Mode 0o600. Daily rotation: snapshots are `events.jsonl.YYYY-MM-DD.gz`; retention is `EVENTS_RETENTION_DAYS = 1` by default (override via `CSW_LOG_RETENTION_DAYS` env). Same retention applies to `startup.log.YYYY-MM-DD.gz` snapshots. Rotated at startup + every 6h timer. The active `events.jsonl` and `startup.log` files are NEVER deleted — only their dated rotation siblings older than the window. |
| `startup.log` | `nohup node dashboard.mjs >>$f` (rc-snippet auto-start + install.sh atomic-block) | Captured stdout/stderr from the dashboard process. Same daily-rotate + same retention as `events.jsonl` — `EVENTS_RETENTION_DAYS = 1` by default (override via `CSW_LOG_RETENTION_DAYS` env). Snapshots `startup.log.YYYY-MM-DD.gz`. Rotation truncates the active file via `writeFileSync(STARTUP_LOG_FILE, '', { mode: 0o600 })` so the open-fd append stream from `nohup` writes to the new active file (the renamed snapshot keeps the historical bytes). |
...
**Spark Q invariants** (round-2 MINOR cleanup — UX2-AC4 / UX2-S5 / UX2-X7 / UX2-WS1 / UX2-AC3 / UX2-X4 / UX2-X6 / UX2-X10):
- `evtIcons` map MUST carry explicit entries for every serialize-mode auto-safeguard event (`serialize-auto-disabled`, `serialize-auto-enabled`, `serialize-auto-reverted`, `serialize-progressive-drain-start`, `serialize-progressive-drain-end`) AND `queue-depth-alert`. Without these the activity-feed icon falls through to the bullet (`•`) and operators lose the visual scan signal during serialize-mode incidents (UX2-AC4 — verified already present).
- The activity tab filter bar (`#activity-filter-bar`) MUST be hidden when `log.length === 0` and shown when `log.length > 0`. The id is part of the public regression contract — `renderActivity()` toggles `style.display` based on the log length so a "Filter activity..." input never sits over a genuinely-empty pane (which misleads users into thinking they have a stale filter hiding everything) (UX2-AC3).
- The four named CSS classes from the audit (`.queue-stats`, `.logs-toolbar`, `.logs-status`, `.logs-clear-btn`, `.miss-row-empty`) MUST own their layout/colour rules instead of letting inline `style="..."` attributes scatter the same triplets across multiple sites. The `.queue-stats` class default is `display: none`; the queue-stats poll MUST set `qEl.style.display = 'block'` (NOT `''`) when serialize is on, because `style.display = ''` only REMOVES the inline attribute and falls back to the class default (which is `display: none`), leaving the line invisible. This was caught by the pre-commit gate as MAJOR before merge. The `.miss-row-empty` class is paired with the existing `.miss-row` so `<div class="miss-row miss-row-empty">…</div>` keeps the structural class plus the modifier (UX2-X6).
- UX2-X10 (the `.fill-full` animation flicker on per-card outerHTML replacement) is addressed BY P's UX-X15 fix above (`pulse-fill 1.5s ease-in-out infinite` plus the prefers-reduced-motion guard). The merged source comment carries both codes (`UX-X15 / UX2-X10`) so any future "let me simplify" refactor that drops the ease-in-out timing reverts both invariants at once. The UX2-X10 spark Q regression test pins the merged ease-in-out rule + dual-code comment.
- `vsFormatStamp` / `vsFormatLabel` / `vsFormatDuration` / `evtTime` / `fmtDurationShort` / `sessionTimeAgo` are deliberately preserved as DISTINCT helpers despite the audit's request to consolidate under `fmtTimestamp(ms, mode)`. They serve genuinely different consumers (filesystem-safe export filenames vs. scrubber labels vs. activity feed time-of-day vs. compact durations vs. relative "ago" strings) — the proposed wrapper would just be a switch over the same 4 implementations. The Spark Q regression test pins every formatter so a future refactor that tries to fold them fails loudly and the contributor sees this rationale (UX2-X4 — skip-with-reason).
- UX2-S5 (sub-5-second sessionTimeAgo gap → "just now") was already addressed by batch L UX-S6; the Spark Q regression pins both the 5000 ms threshold and the literal string so a future refactor cannot silently drop the human-friendly bucket.
- UX2-X7 (`<noscript>` banner uses design-token palette, not hex) was already addressed by batch L UX-X13; the Spark Q regression pins the design-token references AND scopes its hex-absence check to `style="..."` attributes so the explanatory comment that legitimately documents the old hex values for forensic context is not falsely flagged.
- UX2-WS1 (tok-wasted-bar tooltip wraps inside an 18rem box, doesn't flow off-screen) was already addressed by batch L UX-WS5; the Spark Q regression pins `white-space: normal`, `max-width: 18rem`, AND `text-align: center` because dropping any one re-introduces the off-screen-text bug.
