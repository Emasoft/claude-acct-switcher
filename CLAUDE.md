...
## Plugin packaging (Phase H)

A companion plugin `vdm-hooks` lives at `./vdm-hooks/` in this repo — `.claude-plugin/plugin.json` + `hooks/hooks.json` + `README.md` + a self-pointing `marketplace.json`. It packages 17 of the 18 hook subscriptions (everything except the gated `PostToolBatch`) so users can `/plugin install` instead of `install-hooks.sh`-rewriting `~/.claude/settings.json`. The plugin and `install-hooks.sh` are NOT mutually exclusive — both write to the same set of hook URLs and vdm endpoints are idempotent. Most users will keep using `install.sh` (which also handles the daemon, CLI, env-var injection, and Keychain integration); the plugin is for users who specifically want plugin-managed hook lifecycle (e.g. quick `/plugin disable` to silence vdm without uninstalling). The proxy daemon, the `vdm` shell CLI, the credential storage, and the global git hook all stay outside the plugin sandbox — they cannot be plugin-packaged.
